...
# 用法: Rscript tools/render_leningrad.R   （输出到 docs/ln_*.png）
# =====================================================================
...
st <- init_game(gun_key = "B37", test_mode = TRUE, png_dir = "docs")
st <- start_scenario(st, "leningrad")

# 一、初始地图（新建帧，draw_state 写 docs/frame_00N.png）
draw_state(st)
...
cat("已输出地图帧: docs/frame_001.png(初始), docs/frame_002.png(试射后) 等\n")
